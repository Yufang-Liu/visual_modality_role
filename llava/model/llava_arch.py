...
from .multimodal_encoder.builder import build_vision_tower, build_siglip_vision_tower, build_dino_vision_tower
...
build_function_mapping = {'clip-vit-large-patch14-336': build_vision_tower,
                          'clip-vit-base-patch16': build_vision_tower,
                        'siglip-base-patch16-224': build_siglip_vision_tower,
                        'siglip-so400m-patch14-384': build_siglip_vision_tower,
                        'dinov2-large': build_dino_vision_tower,
                        'dino-vitb16': build_dino_vision_tower,
                        }
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            if ',' not in config.mm_vision_tower and '.' not in config.mm_vision_tower:
                self.vision_tower = build_function_mapping[config.mm_vision_tower.split('/')[-1]](config)
                ...
                for v in vision_tower_name_split:
                    ...
                    if 'noise' in v:
                        ...
                        list_vision_towers.append(build_function_mapping[v.split('/')[-1]](config))
                ...
                for v in vision_tower_name_split:
                    ...
                    if not hasattr(config, "image_size"):
                        config.image_size = list_vision_towers[-1].config.image_size
                self.vision_tower = list_vision_towers
                config.mm_vision_tower = ori_mm_vision_tower

            if '.' not in config.mm_vision_tower or type(config.mm_hidden_size) is not list:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if ',' not in vision_tower_name and '.' not in vision_tower_name:
            ...
            vision_tower = build_function_mapping[vision_tower_name.split('/')[-1]](model_args)
            ...
            for v in vision_tower_name_split:
                if v == 'noise':
                    ...
                    list_vision_towers.append(build_function_mapping[v.split('/')[-1]](model_args))
            ...
            for v in vision_tower_name_split:
                if v == 'noise':
                    ...
                    model_args.vision_tower = v
                    list_vision_towers.append(build_function_mapping[v.split('/')[-1]](model_args))
                if not hasattr(model_args, "image_size"):
                    model_args.image_size = list_vision_towers[-1].config.image_size
            vision_tower = list_vision_towers
            self.config.mm_vision_tower = vision_tower_name
            model_args.vision_tower = vision_tower_name
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        if type(images) is not list:
            ...
            mm_projector = self.get_model().get_mm_projector()
            vision_tower = self.get_model().get_vision_tower()
            if type(vision_tower) is nn.ModuleList:
                if not type(mm_projector) is nn.ModuleList:
                    f_list = []
                    ...
                    for i in range(len(images)):
                        if not images[i] is None:
                            image_features = vision_tower[i](images[i])
                            image_features_list.append(mm_projector[i](image_features))
